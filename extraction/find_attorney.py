...
class AttorneyFinder:
    def __init__(
        ...
        *,
        ...
        # Track extraction failures for reporting
    ...
    def run(self, excel_path: str) -> int:
        ...
        for firm, url in firms:
            try:
                ...
                # Save JSON output
                self._save_json_output(firm, attorneys)
        ...
        # Save extraction failure report
        if self.extraction_failures:
            self._save_extraction_failure_report()
        
        return total_processed

    def _filter_us_offices(self, attorneys: list[dict]) -> list[dict]:
        """Filter to US offices only using config/us_states.json"""
        try:
            config_path = Path(__file__).parent.parent / "config" / "us_states.json"
            with open(config_path, 'r', encoding='utf-8') as f:
                us_data = json.load(f)
            
            us_states = set(us_data["states"])
            us_state_names = set([name.lower() for name in us_data["state_names"]])
            
            filtered = []
            for att in attorneys:
                office = att.get("office", "").lower()
                
                if not office:
                    # No office info - include by default
                    filtered.append(att)
                    continue
                
                # Check if office contains US state abbrev or name
                is_us = False
                for state in us_states:
                    if state.lower() in office or f", {state.lower()}" in office:
                        is_us = True
                        break
                
                if not is_us:
                    for state_name in us_state_names:
                        if state_name in office:
                            is_us = True
                            break
            ...
            self.log(f"  Warning: US office filtering failed: {e}")
            return attorneys
    ...
    def _save_json_output(self, firm_name: str, attorneys: list[dict]) -> None:
        """Save attorneys data to JSON"""
        outputs_dir = Path("outputs")
        outputs_dir.mkdir(exist_ok=True)
        
        safe_name = re.sub(r'[^\w\s-]', '', firm_name).strip().replace(' ', '_')
        json_path = outputs_dir / f"attorneys_{safe_name}.json"
    ...
    def _save_extraction_failure_report(self) -> None:
        """Save extraction failures to Excel report"""
        from openpyxl import Workbook
        
        outputs_dir = Path("outputs")
        outputs_dir.mkdir(exist_ok=True)
        report_path = outputs_dir / "extraction_failure_report.xlsx"
        
        wb = Workbook()
        ws = wb.active
        ws.title = "Extraction Failures"
        ws.append(["Firm", "Profile URL", "Status Code", "Failure Reason", "Timestamp"])
        ...
        wb.save(report_path)
        self.log(f"  Extraction failure report saved: {report_path} ({len(self.extraction_failures)} failures)")
    ...
    def process_firm(self, firm_name: str, base_url: str) -> tuple[list[dict], DiscoveryMetrics]:
        ...
        # Check if using pre-validated sources
        if self.sources_file:
            attorneys = self._try_multi_source_path(firm_name, base_url, metrics)
        else:
            # FAST PATH: Sitemap + requests
            attorneys = self._try_fast_path(base_url, metrics, firm_name)
            ...
            if len(attorneys) < MIN_ATTORNEYS_THRESHOLD:
                ...
                playwright_attorneys, metrics = self._try_playwright_path(base_url, metrics, firm_name)
        ...
        # Filter to US offices only
        unique = self._filter_us_offices(unique)
    ...
    def _try_fast_path(self, base_url: str, metrics: DiscoveryMetrics, firm_name: str = "") -> list[dict]:
        ...
        try:
            ...
            if profile_urls:
                ...
                with ThreadPoolExecutor(max_workers=8) as executor:
                    futures = {
                        executor.submit(self._fetch_profile_metadata, url, firm_name): url
    ...
    def _try_multi_source_path(self, firm_name: str, base_url: str, metrics: DiscoveryMetrics) -> list[dict]:
        """Multi-source path: Use pre-validated sources from firm_finder"""
        attorneys = []
        ...
        try:
            ...
            sources_col = headers.index("attorney_list_sources") + 1
            types_col = headers.index("source_types") + 1
            
            firm_row = None
            for row in range(2, ws.max_row + 1):
                if ws.cell(row, firm_col).value == firm_name:
                    firm_row = row
                    break
            
            if not firm_row:
                self.log(f"  ⚠ Firm not found in sources file")
                return attorneys
            
            sources = ws.cell(firm_row, sources_col).value
            types = ws.cell(firm_row, types_col).value
            
            wb.close()
            
            if not sources:
                self.log(f"  ⚠ No validated sources for this firm")
                return attorneys
            
            # Parse pipe-separated sources
            source_urls = sources.split("|")
            source_types = types.split("|") if types else ["unknown"] * len(source_urls)
            
            self.log(f"  Using {len(source_urls)} pre-validated sources")
            
            # Gather URLs from all sources
            all_profile_urls = set()
            for source_url, source_type in zip(source_urls, source_types):
                self.log(f"    [{source_type}] {source_url[:60]}...")
                
                if source_type == "sitemap":
                    urls = self._extract_profile_urls_from_sitemap_direct(source_url, base_url)
                elif source_type in ["directory", "alphabet"]:
                    urls = self._extract_profile_urls_from_directory(source_url, base_url)
                else:
                    urls = []
                
                all_profile_urls.update(urls)
            
            self.log(f"  Gathered {len(all_profile_urls)} profile URLs from all sources")
            
            # Parallel enrichment
            with ThreadPoolExecutor(max_workers=8) as executor:
                futures = {
                    executor.submit(self._fetch_profile_metadata, url, firm_name): url
                    for url in list(all_profile_urls)[: self.limit * 3 if self.limit > 0 else len(all_profile_urls)]
                }
                for future in as_completed(futures):
                    try:
                        att = future.result()
            ...
            self.log(f"  Multi-source path error: {e}")
            metrics.failure_notes.append(f"Multi-source error: {e}")
        
        return attorneys

    def _extract_profile_urls_from_sitemap_direct(self, sitemap_url: str, base_url: str) -> list[str]:
        """Extract profile URLs from a sitemap URL directly"""
        # Reuse existing method but with direct URL
        try:
            ...
            if is_index:
                # Get sub-sitemaps for people
                for elem in root.iter():
                    if elem.tag.endswith("}loc") or elem.tag == "loc":
                        sub_url = elem.text
                        if sub_url and any(kw in sub_url.lower() for kw in ['people', 'professional', 'attorney', 'lawyer']):
                            urls.extend(self._extract_profile_urls_from_sitemap_direct(sub_url, base_url))
            else:
                # Regular sitemap
                for elem in root.iter():
                    if elem.tag.endswith("}loc") or elem.tag == "loc":
                        url = elem.text
                        if url:
                            # Canonicalize cm-prod domains BEFORE checking
                            from urllib.parse import urlparse
                            parsed = urlparse(url)
                            base_parsed = urlparse(base_url)
                            
                            # Replace bad domains (cm-prod, etc.)
                            bad_domains = ['cm-prod', 'staging', 'dev', 'akamaized', 'cloudfront', 'azureedge']
                            if any(bad in parsed.netloc for bad in bad_domains):
                                url = url.replace(parsed.netloc, base_parsed.netloc)
                            
                            if self._is_profile_url(url, base_url):
                                urls.append(url)
    ...
    def _extract_profile_urls_from_directory(self, dir_url: str, base_url: str) -> list[str]:
        """Extract profile URLs from directory/alphabet page"""
        urls = []
        try:
            resp = self.session.get(dir_url, timeout=REQUEST_TIMEOUT)
            if resp.status_code != 200:
                return urls
    ...
    def _is_profile_url(self, url: str, base_url: str) -> bool:
        ...
        try:
            ...
            profile_keywords = ['/lawyer', '/attorney', '/people/', '/professional', '/bio/', '/profile/', '/team/', '/our-people/']
            if not any(kw in url_lower for kw in profile_keywords):
                return False
            
            # Must not have junk patterns
            junk = ['terms', 'privacy', 'advertising', 'disclaimer', 'cookie', '/search', 'login', 'subscribe', 'career', 'alumni']
            if any(j in url_lower for j in junk):
                return False
    ...
    def _try_playwright_path(
        self, base_url: str, metrics: DiscoveryMetrics, firm_name: str = ""
        ...
        attorneys = self._enrich_profile_urls(list(all_profile_urls), firm_name)
    ...
    def _enrich_profile_urls(self, urls: list[str], firm_name: str = "") -> list[dict]:
        """Fetch metadata for all profile URLs"""
        attorneys = []
        
        with ThreadPoolExecutor(max_workers=self.workers) as executor:
            futures = {executor.submit(self._fetch_profile_metadata, url, firm_name): url for url in urls}
            for future in as_completed(futures):
                try:
                    att = future.result()
                    if att and att.get("name"):
    ...
    def _fetch_profile_metadata(self, url: str, firm_name: str = "") -> dict | None:
        """Fetch metadata from profile page with Playwright accordion enhancement"""
        try:
            ...
            if resp.status_code == 403 or "cloudflare" in resp.text.lower():
                self._log_extraction_failure(firm_name, url, resp.status_code, "Cloudflare blocked")
                # Try external fallback
                return self._try_external_fallback(url, firm_name)
            
            if resp.status_code != 200:
                self._log_extraction_failure(firm_name, url, resp.status_code, f"HTTP {resp.status_code}")
                return None
            ...
            # If critical fields missing, try Playwright with accordion clicking
            if not name or (not title and not practice and not office):
                enriched = self._try_playwright_enrichment(url)
                if enriched:
                    name = enriched.get("name") or name
                    title = enriched.get("title") or title
                    practice = enriched.get("practice") or practice
                    office = enriched.get("office") or office

            if not name:
                self._log_extraction_failure(firm_name, url, 200, "Name extraction failed")
                return None
            ...
            self._log_extraction_failure(firm_name, url, 0, f"Exception: {str(e)}")
            return None
    ...
    def _log_extraction_failure(self, firm_name: str, url: str, status_code: int, reason: str) -> None:
        """Log extraction failure for reporting"""
    ...
    def _try_playwright_enrichment(self, url: str) -> dict | None:
        """Use Playwright to click accordions and extract hidden content"""
        try:
            ...
            with sync_playwright() as p:
                ...
                # Keywords for accordion/tab elements
                accordion_keywords = ["education", "admissions", "bar", "credentials", "biography", "background"]
                
                # Try to click accordion/tab buttons
                for keyword in accordion_keywords:
                    selectors = [
    ...
    def _try_external_fallback(self, url: str, firm_name: str) -> dict | None:
        """Fallback to external directories when official site blocks"""
        # Extract attorney name from URL if possible
        path = urlparse(url).path
        segments = [s for s in path.split("/") if s]
        ...
        last_segment = segments[-1].replace("-", " ").replace("_", " ").title()
        
        if not self._looks_like_person_name(last_segment):
            return None
        
        # TODO: Implement Martindale/SuperLawyers API fallback
        # For now, return partial result with note
        self.log(f"  External fallback needed for: {url} (not implemented)")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--sources-file",
        default="",
        help="Path to validated sources.xlsx from firm_finder (uses pre-validated sources instead of discovery)",
    )
