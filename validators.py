...
# Some firm templates render the name node as "Contact<FullName><Title>" with no
# whitespace separators (notably Proskauer's professionals pages, e.g.
# "ContactAaron M. FrancisAssociate"). This regex peels off the leading
# "Contact" prefix and the trailing role token(s) so only the actual name
# remains. Kept generic (not gated on firm name) per AGENTS.md - the same
# pattern can recover any firm that exposes the same DOM shape.
_NAME_CONTACT_WRAPPER_RE = re.compile(
    r"^Contact"
    r"(?P<name>.+?[a-zà-öø-ÿ\u0100-\u024F\.])"
    r"(?P<title>"
        r"(?:[A-Z][a-z]+\s+){0,5}"
        r"(?:Associate|Partner|Counsel|Attorney|Clerk|Specialist|"
        r"Director|Principal|Paralegal|Advisor|Consultant|Fellow|"
        r"Officer|Chair|Chairman|Emeritus|Manager|Head|Chief)"
        r"(?:\s+[A-Za-z][\w\s&,'-]*)?"
    r")$"
)
_NAME_CONTACT_BARE_RE = re.compile(r"^Contact(?P<name>[A-Z].+)$")
...
def validate_name(raw: str | None) -> tuple[str | None, str | None]:
    ...
    wrapper_match = _NAME_CONTACT_WRAPPER_RE.match(name)
    if wrapper_match:
        name = wrapper_match.group("name").strip()
    else:
        bare_match = _NAME_CONTACT_BARE_RE.match(name)
        if bare_match:
            name = bare_match.group("name").strip()
