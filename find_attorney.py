...
def _safe_browser_close(browser: Any) -> None:
    """Close a Playwright browser with a 10-second timeout to prevent hanging."""
    import threading as _threading
    ct = _threading.Thread(target=browser.close, daemon=True)
    ct.start()
    ct.join(timeout=10)
...
def _extract_hrefs(html: str | bytes, limit: int = 500) -> list[str]:
    """Extract href values from raw HTML/bytes via regex (no parser overhead)."""
    if isinstance(html, bytes):
        html = html.decode('utf-8', errors='ignore')
    return _HREF_PATTERN.findall(html)[:limit]


def _block_heavy_resources(context, *, block_stylesheet: bool = True) -> None:
    """Register a Playwright route that aborts heavy resource fetches."""
...
_API_URL_KEYWORDS = ("api", "graphql", "search", "people", "attorneys", "lawyers", "professionals")


def _make_api_response_handler(
    ...
    *,
    ...
    def handle_response(response):
        try:
            ...
            if not any(kw in url_lower for kw in _API_URL_KEYWORDS):
                return
            if not response.ok:
                return
            ...
            try:
                data = response.json()
                records = extract_records(data, base_url)
                if records and log_fn is not None:
                    log_fn(f"  API detected: {response.url}, {len(records)} records")
                entry = {"url": response.url, "records": records}
                if store_json:
                    entry["json"] = data
                if records or store_json:
                    captured_api_data.append(entry)
            except Exception:
                pass
        except Exception:
            pass
...
class AttorneyFinder:
    ...
    def _html_sitemap_has_profile_links(self, sitemap_url: str, base_url: str, domain: str) -> bool:
        ...
        try:
            ...
            import re
            links = _extract_hrefs(content, limit=300)
    ...
    def _probe_directory_type(self, base_url: str) -> dict:
        ...
        try:
            with sync_playwright() as p:
                ...
                handle_response = _make_api_response_handler(
                    captured_api_data, self._extract_attorneys_from_json, base_url,
                )
    ...
    def run(self, excel_path: str) -> int:
        ...
        try:
            ...
            corrections_path = os.path.join(os.path.dirname(os.path.abspath(excel_path)) or ".", "url_corrections.json")
            try:
                with open(corrections_path, encoding="utf-8") as _cf:
                    url_corrections = json.load(_cf)
                self.log(f"Loaded {len(url_corrections)} URL corrections from {corrections_path}")
            except FileNotFoundError:
                pass
            except Exception as _ce:
                self.log(f"  [WARN] Failed to load url_corrections.json: {_ce}")

            if self.sheet_name in wb.sheetnames:
                del wb[self.sheet_name]
            out_ws = wb.create_sheet(self.sheet_name)
            from attorney_extractor import split_education_by_degree
            out_ws.append(
                [
                    "Firm",
                    "Attorney Name",
            ...
            for row in ws.iter_rows(min_row=2, values_only=False):
                ...
                if firm_name in url_corrections:
                    corrected = url_corrections[firm_name]
                    if corrected and corrected != base_url:
                        self.log(f"  [URL_CORRECTION] {firm_name}: {base_url} → {corrected}")
                        base_url = corrected
                ...
                try:
                    ...
                    for att in attorneys:
                        ...
                        if hasattr(att, 'full_name'):  # New AttorneyProfile format
                            ...
                            jd_str, ba_str, llm_str = split_education_by_degree(att.education)
    ...
    def _fetch_urls_from_source_with_tracking(
        ...
        try:
            ...
            if method == "xml_sitemap":
                ...
                import re
                links = _extract_hrefs(response.text, limit=500)
                for link in links:
    ...
    def _quick_extract_bar_admissions(self, soup, text: str) -> list[str]:
        ...
        from attorney_extractor import US_STATES
        text_lower = text.lower()
        return [state for state in US_STATES if len(state) > 3 and state.lower() in text_lower]
        ...
        try:
            ...
            if len(profile_urls) > 0:
                ...
                validation_result = validate_source(
                    ...
                    sample_size=min(8, len(profile_urls)),
            ...
            for sitemap_url in sitemap_urls:
                if self._classify_sitemap(sitemap_url, domain) == "HTML":
                    ...
                    if resp.status_code == 200:
                        ...
                        import re
                        links = _extract_hrefs(resp.text, limit=500)
                        for link in links:
                            full_url = urljoin(base_url, link)
                            if self._is_profile_like_url(full_url, domain):
                                urls.add(full_url)
    ...
    def _strategy_kirkland_scroll(self, base_url: str) -> tuple[set[str], int | None]:
        ...
        try:
            with sync_playwright() as p:
                ...
                _block_heavy_resources(context, block_stylesheet=False)
                pg = context.new_page()
    ...
    def _try_playwright_path(
        ...
        with sync_playwright() as p:
            ...
            handle_response = _make_api_response_handler(
                captured_api_data, self._extract_attorneys_from_json, base_url,
                store_json=True, log_fn=self.log,
            )
    ...
    def _exhaust_with_stabilization(
        self, page, base_url: str, metrics: DiscoveryMetrics, *,
        max_iter: int, action, log_label: str, log_unit: str,
        metric_attr: str, iter_to_display=lambda i: i,
    ) -> set[str]:
        all_urls: set[str] = set()
        stabilization_counter = 0
        iterations = 0

        while stabilization_counter < self.stabilization and iterations < max_iter:
            try:
                prev_count = len(all_urls)
                all_urls.update(self._extract_profile_urls_from_page(page, base_url))

                if not action(page, iterations):
                    break
                iterations += 1
        ...
        setattr(metrics, metric_attr, len(all_urls))
        count = getattr(metrics, metric_attr)
        if count > 0:
            self.log(f"  {log_label}: {count} URLs ({iter_to_display(iterations)} {log_unit})")
        return all_urls
    ...
    def _handle_pagination_stabilized(
        ...
        return self._exhaust_with_stabilization(
            page, base_url, metrics,
            max_iter=50, action=action,
            log_label="Pagination", log_unit="pages",
            metric_attr="discovered_by_pagination",
            iter_to_display=lambda i: i + 1,
        )
    ...
    def _handle_loadmore_stabilized(
        ...
        return self._exhaust_with_stabilization(
            page, base_url, metrics,
            max_iter=30, action=action,
            log_label="Load-more", log_unit="clicks",
            metric_attr="discovered_by_loadmore",
        )
    ...
    def _handle_scroll_stabilized(
        ...
        return self._exhaust_with_stabilization(
            page, base_url, metrics,
            max_iter=20, action=action,
            log_label="Scroll", log_unit="scrolls",
            metric_attr="discovered_by_scroll",
        )
    ...
    def _enrich_profile_urls(self, urls: list[str], base_url: str, firm_name: str) -> list[dict]:
        ...
        def _run_batch_shared_browser(
            ...
            def _worker(sub_batch: list) -> None:
                ...
                try:
                    with sync_playwright() as p:
                        # Memory-safe Chromium launch: prevent /dev/shm exhaustion
                        # and cap V8 heap so a single tab can't balloon the host.
                        ...
                        # Block heavy resources we never parse (images, fonts,
                        # media, stylesheets) — typically 70%+ memory savings
                        # across hundreds of profile navigations.
    ...
    def _extract_profile_urls_from_sitemap(self, base_url: str) -> list[str]:
        ...
        # Check robots.txt for sitemap directives. Critical for firms whose
        # actual sitemap lives at a non-standard path (e.g. Norton Rose's
        # /sitemap-NRFWeb.xml). Failures here are silent in production but
        # we log them so they show up in --debug-firm runs.
        sitemap_urls = []
        try:
            robots_url = f"{scheme}://{domain}/robots.txt"
            self.rate_limit_manager.wait(domain)
            ...
            if resp.status_code == 200:
                for line in resp.text.split('\n'):
                    if line.lower().startswith('sitemap:'):
                        sitemap_url = line.split(':', 1)[1].strip()
                        sitemap_urls.append(sitemap_url)
                self.log(f"  [SITEMAP_DEBUG] robots.txt: found {len(sitemap_urls)} Sitemap directive(s)")
            else:
                self.log(f"  [SITEMAP_DEBUG] robots.txt fetch returned {resp.status_code}")
        except Exception as _rex:
            self.log(f"  [SITEMAP_DEBUG] robots.txt fetch error: {_rex}")

        # Always probe attorney-specific sitemap paths in addition to whatever
        # robots.txt lists or the generic defaults — many firms (Reed Smith,
        # Akin, etc.) keep attorney URLs in a dedicated sitemap that is NOT
        # advertised in robots.txt and NOT included in the generic /sitemap.xml.
        attorney_specific_paths = [
            "/people-sitemap.xml",
            "/sitemap-people.xml",
            "/sitemap_people.xml",
            "/professionals-sitemap.xml",
            "/sitemap-professionals.xml",
            "/sitemap-attorneys.xml",
            "/attorneys-sitemap.xml",
            "/sitemap-lawyers.xml",
            "/lawyers-sitemap.xml",
            "/sitemap-team.xml",
        ]
        for path in attorney_specific_paths:
        ...
        for default_path in ("/sitemap.xml", "/sitemap_index.xml"):
            candidate = f"{scheme}://{domain}{default_path}"
            if candidate not in sitemap_urls:
                sitemap_urls.append(candidate)
        ...
        for sitemap_url in sitemap_urls:
            try:
                ...
                resp = self.session.get(sitemap_url, timeout=REQUEST_TIMEOUT)
                if resp.status_code != 200:
                    self.log(f"  [SITEMAP_DEBUG] {sitemap_url} → HTTP {resp.status_code}")
                    failed_sitemap_urls.append(sitemap_url)
                    continue
        ...
        if not all_profile_urls and failed_sitemap_urls:
            self.log(f"  [SITEMAP_FALLBACK] requests fetch failed for {len(failed_sitemap_urls)} sitemap URL(s); trying Playwright fallback")
            for sitemap_url in failed_sitemap_urls:
                content = self._fetch_sitemap_via_playwright(sitemap_url)
                if not content:
                    continue
                try:
                    root = ET.fromstring(content)
                    self._parse_sitemap_recursive(root, all_profile_urls, base_url, max_urls=max(self.limit * 10, 500) if self.limit > 0 else 5000)
                    if all_profile_urls:
                        self.log(f"  [SITEMAP_FALLBACK] Playwright recovered {len(all_profile_urls)} URLs from {sitemap_url}")
                        break
                except Exception as _ex:
                    self.log(f"  [SITEMAP_FALLBACK] parse error after Playwright fetch of {sitemap_url}: {_ex}")
                    continue

        return list(all_profile_urls)

    def _fetch_sitemap_via_playwright(self, sitemap_url: str) -> bytes | None:
        """Fetch a sitemap XML payload via headless Chromium when requests is blocked.

        Returns raw bytes on success, None if Playwright is unavailable or the fetch fails.
        Used only as a last-resort fallback for firms whose sitemaps are gated by
        Cloudflare/edge rules that block bare HTTP clients but allow real browsers.
        """
        try:
            from playwright.sync_api import sync_playwright
        except ImportError:
        ...
        try:
            with sync_playwright() as p:
                ...
                context = browser.new_context()
                try:
                    response = context.request.get(sitemap_url, timeout=15000)
            ...
            self.log(f"  [SITEMAP_FALLBACK] Playwright fetch failed for {sitemap_url}: {_ex}")
            return None
    ...
    def _is_attorney_profile_url(self, url: str) -> bool:
        ...
        profile_segments = {"lawyers", "lawyer", "attorney", "attorneys", "people", "professionals",
                            ...
                            "our-people", "our-lawyers", "our-attorneys", "our-professionals",
                            "the-team", "members", "biographies"}
