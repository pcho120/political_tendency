...
# Maps various degree spellings to canonical column buckets (JD / BA / LLM).
_DEGREE_COLUMN_MAP = {
    "JD": "JD", "J.D.": "JD",
    "BA": "BA", "B.A.": "BA",
    "LLM": "LLM", "LL.M.": "LLM", "LL.M": "LLM",
}


def split_education_by_degree(
    records: "list[EducationRecord] | list[dict]",
) -> tuple[str, str, str]:
    """Split education records into (JD, BA, LLM) display strings.

    Each cell formatted as ``"<school>, <year>"`` (year omitted if missing).
    Multiple entries with the same degree are joined with ``" | "``.
    Degrees outside JD / BA / LLM are ignored.
    """
    buckets: dict[str, list[str]] = {"JD": [], "BA": [], "LLM": []}
    for r in records or []:
        if isinstance(r, dict):
            ...
            degree = (r.degree or "").strip()
            school = (r.school or "").strip()
            year = r.year
        key = _DEGREE_COLUMN_MAP.get(degree) or _DEGREE_COLUMN_MAP.get(degree.upper())
        if not key:
            continue
        if school and year:
            cell = f"{school}, {year}"
...
class AttorneyExtractor:
    ...
    def _extract_offices_bs4(self, soup: BeautifulSoup) -> list[str]:
        ...
        # Strategy 0c: Sullivan & Cromwell — <div class='bio-loc'>New York +1-212-...</div>
        bio_loc = soup.find(class_='bio-loc')
        if bio_loc:
            raw = bio_loc.get_text(' ', strip=True)
            city = re.split(r'\s*(?:\+?\d|t:|tel|phone)', raw, maxsplit=1, flags=re.I)[0].strip(' ,')
            if city and 2 < len(city) < 60:
                offices.append(city)
                return offices

        # Strategy 0d: Simpson Thacher — <div class='hero-area-address2'>Palo Alto, CA 94304</div>
        for cls_name in ('hero-area-address2', 'hero-area-address1', 'hero-area-location'):
            el = soup.find(class_=cls_name)
            if el:
                raw = el.get_text(' ', strip=True)
                first = re.split(r'\d{5}', raw)[0].strip(' ,')
                if first and len(first) < 80:
                    offices.append(first)
                    break
        if offices:
            return offices

        # Strategy 0e: anchor href to /office(s)/{slug}/ or /locations/.../{slug}.html
        # Catches Baker & Hostetler, Dechert, Husch Blackwell, Troutman before noisy class regex
        href_office_pat = re.compile(r'/offices?/[a-z0-9-]+(?:/|\.html)?$|/locations?/[a-z0-9/.-]+(?:/|\.html)?$', re.I)
        for a in soup.find_all('a', href=True):
            href = a.get('href', '')
            if not href_office_pat.search(href):
                continue
            ...
            if not text or len(text) > 60:
                continue
            if text.lower() in {'locations', 'offices', 'our offices', 'all offices', 'find an office', 'office locations'}:
                continue
            if text not in offices:
                offices.append(text)
        if offices:
            return offices

        # Reject obvious UI labels that the broad class regex would otherwise capture
        UI_NOISE = frozenset({
            'arrow down', 'arrow up', 'arrow', 'search', 'menu', 'close', 'open',
            'toggle', 'expand', 'collapse', 'filter', 'sort', 'select',
            'next', 'previous', 'prev', 'back', 'home', 'more',
            'show more', 'show less', 'view all', 'see all', 'load more',
            'language', 'languages', 'login', 'sign in', 'sign up', 'register',
            'submit', 'cancel', 'ok', 'yes', 'no',
    ...
    def _extract_degree_from_text(self, text: str) -> str | None:
        from validators import extract_degree_from_text
        return extract_degree_from_text(text)

    def _extract_year_from_text(self, text: str) -> int | None:
        from validators import extract_year_from_text
