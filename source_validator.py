...
class SourceValidator:
    ...
    def validate_source(self, 
                       ...
                       sample_size: int = 8) -> SourceValidationResult:
        """
        ...
        # Relaxed: name still required across all usable samples, but enrichment
        # fields (title/office/practice_areas) only need 1 hit in any sample.
        # Many firms render those fields via JS or use uncommon class names that
        # the lightweight sampler misses — Playwright enrichment will recover them later.
        name_required = effective_sample_count  # All successful samples must have name
        field_threshold = max(1, effective_sample_count // 8)  # At least 1 hit
        ...
        # Practice areas or office found at threshold is sufficient corroboration
        # that the source is a real attorney directory; title alone is a fragile
        # signal because the lightweight regex misses many firm templates.
        if passed_practice or passed_office:
        ...
        if not passed_name:
            ...
            # Retry once with a fresh random sample before giving up.
            # Many firms have inconsistent profile templates — a second draw often hits a profile with title.
            retry_urls = [u for u in candidate_urls if u not in {s.url for s in successful_samples + failed_samples}]
            if retry_urls:
                retry_pool = random.sample(retry_urls, min(pool_size, len(retry_urls)))
                logger.info(f"  Title threshold not met; retrying with fresh pool of {len(retry_pool)} URLs...")
                retry_successful: list = []
                for url in retry_pool:
                    s = self._sample_profile(url)
                    if s.name or s.extraction_success:
                        retry_successful.append(s)
                        if len(retry_successful) >= sample_count:
                            break
                if retry_successful:
                    successful_samples.extend(retry_successful)
                    effective_sample_count = len(successful_samples)
                    field_counts = {
                        'name': sum(1 for s in successful_samples if s.name),
                        'title': sum(1 for s in successful_samples if s.title),
                        'office': sum(1 for s in successful_samples if s.has_us_office),
                    ...
                    result.field_validation = field_counts
                    result.sampled_profiles = successful_samples + failed_samples
                    name_required = effective_sample_count
                    field_threshold = max(1, effective_sample_count // 8)
                    passed_name = field_counts['name'] >= name_required
                    passed_title = field_counts['title'] >= field_threshold
                    allow_missing_title = any(s.from_playwright and s.name for s in successful_samples) or allow_missing_title
                    logger.info(f"  After retry: name={field_counts['name']}/{effective_sample_count}, title={field_counts['title']}/{effective_sample_count}")

            if passed_name and (passed_title or allow_missing_title):
                result.is_valid = True
                if not passed_title and allow_missing_title:
                    result.validation_notes.append("title_missing_in_sample")
                result.validation_notes.append(f"Source validated after retry: {field_counts['name']} names, {field_counts['title']} titles")
            else:
                result.failure_reason = "title_threshold_not_met"
                result.validation_notes.append(f"Title field below threshold after retry: {field_counts['title']}/{effective_sample_count}")
        else:
            result.is_valid = True
            if not passed_title and allow_missing_title:
                result.validation_notes.append("title_missing_in_sample")
